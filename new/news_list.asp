...
if Request("type") = "sidebar" then
	if lang = "chi" then
		sql = "SELECT TOP 20 id, title_chi as title FROM article WHERE (((" & category_code & " + 1 \ (category_code + 1) ) mod 2) = 1) AND publish = true ORDER BY sequence_indexed DESC"
	elseif lang = "bm" then
		sql = "SELECT TOP 20 id, title_bm as title FROM article WHERE (((" & category_code & " + 1 \ (category_code + 1) ) mod 2) = 1) AND publish = true ORDER BY sequence_indexed DESC"
	else
		sql = "SELECT TOP 20 id, title FROM article WHERE (((" & category_code & " + 1 \ (category_code + 1) ) mod 2) = 1) AND publish = true ORDER BY sequence_indexed DESC"
	end if
elseif Request("type") = "" then
	...
	sql = "SELECT Count(article.id) AS total FROM article WHERE (((" & category_code & " + 1 \ (category_code + 1) ) mod 2) = 1) AND publish = true"
	...
	if lang = "chi" then
		sql = "SELECT TOP " & OFFSET &  " * FROM ( SELECT * FROM ( SELECT TOP " & postCounterAfterPaginated & " * FROM ( SELECT id, title_chi as title, sequence_indexed FROM article ta WHERE (((" & category_code & " + 1 \ (category_code + 1) ) mod 2) = 1) AND publish = true ORDER BY ta.sequence_indexed ASC) ) subTa ORDER BY subTa.sequence_indexed DESC )"
	elseif lang = "bm" then
		sql = "SELECT TOP " & OFFSET &  " * FROM ( SELECT * FROM ( SELECT TOP " & postCounterAfterPaginated & " * FROM ( SELECT id, title_bm as title, sequence_indexed FROM article ta WHERE (((" & category_code & " + 1 \ (category_code + 1) ) mod 2) = 1) AND publish = true ORDER BY ta.sequence_indexed ASC) ) subTa ORDER BY subTa.sequence_indexed DESC )"
	else
		sql = "SELECT TOP " & OFFSET &  " * FROM ( SELECT * FROM ( SELECT TOP " & postCounterAfterPaginated & " * FROM ( SELECT id, title, sequence_indexed FROM article ta WHERE (((" & category_code & " + 1 \ (category_code + 1) ) mod 2) = 1) AND publish = true ORDER BY ta.sequence_indexed ASC) ) subTa ORDER BY subTa.sequence_indexed DESC )"
	end if

else
	if lang = "chi" then
		sql = "SELECT TOP 20 article_id as id, title_chi as title, index_image_url as preview_image_url FROM article WHERE (((" & category_code & " + 1 \ (category_code + 1) ) mod 2) = 1) AND publish = true ORDER BY sequence_indexed DESC"
	elseif lang = "bm" then
		sql = "SELECT TOP 20 article_id as id, title_bm as title, index_image_url as preview_image_url FROM article WHERE (((" & category_code & " + 1 \ (category_code + 1) ) mod 2) = 1) AND publish = true ORDER BY sequence_indexed DESC"
	else
		sql = "SELECT TOP 20 article_id as id, title, index_image_url as preview_image_url FROM article WHERE (((" & category_code & " + 1 \ (category_code + 1) ) mod 2) = 1) AND publish = true ORDER BY sequence_indexed DESC"
	end if
end if
